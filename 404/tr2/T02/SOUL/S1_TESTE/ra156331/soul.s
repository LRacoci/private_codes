...
alarms_handler:
	...
	@ Corpo do for
		...
		if_1:
			...
			stmfd sp!, {r0-r6}		@ Salva o contexto


			msr CPSR_c, #USR_MODE_I_0_F_0	@ Muda para modo usuario

			stmfd sp!, {r0-r12, lr}		@ Salva o contexto do usuario anterior
			blx r5						@ Pula para o codigo do usuario
			ldmfd sp!, {r0-r12, lr}		@ Recuperao contexto antigo

			mov r0, #IRQ_MODE_I_1_F_1		@ Parametro da syscall que e o proximo modo
			ldr r1, =end_svc_alarms_handler	@ Parametro que indica para onde voltar
			mov r7, #23						@ Identificador da syscall
			svc 0x0

			end_svc_alarms_handler:

			ldmfd sp!, {r0-r6}		@ Recupera o contexto
...
callbacks_handler:
	...
	@ Corpo do for
		...
		if_3:
			...
			stmfd sp!, {r0-r12, lr} 	@ Salva o contexto do usuario
			blx r6						@ Pula para o codigo do usuario
			ldmfd sp!, {r0-r12, lr}		@ Recuperao o contexto do usuário

			mov r0, #IRQ_MODE_I_1_F_1			@ Parametro da syscall que e o proximo modo
			ldr r1, =end_svc_callbacks_handler	@ Parametro que indica para onde voltar
			mov r7, #23							@ Identificador da syscall
			svc 0x0

			end_svc_callbacks_handler:
...
read_sonar:						@ (r0) : unsigned char 		sonar_id, 
	...
	loop_flag:
		...
		bl delay_sonar_10ms	@ Chama o delay enquanto flag != 1
...
back_to_r0:						@ 	(r0) : status register save 	SPSR
								@	(r1) : unsigned int 			endereco
	@ Ainda em modo SUPERVISOR, desempilha os registradores salvos no inicio
	ldmfd sp!, {r11}		
	ldmfd sp!, {r1-r12, lr}
	@ Grava no registrador de status o argumento da syscall e muda de modo
	msr CPSR_c, r0
	mov pc, r1
...
delay_sonar_15ms:				@ 	Nao tem parametros
	...
	ldr r4, =0x4000				@ Inicializa o iterador com o valor maximo 0x1400
	loop_delay_15:
...
delay_sonar_10ms:					@ 	Nao tem parametros
	...
	ldr r4, =0x3000				@ Inicializa o iterador com o valor maximo 0x1400
	loop_delay_10:
		sub r4, r4, #1			@ Subtrai 1
		cmp r4, #0				@ Compara com 0

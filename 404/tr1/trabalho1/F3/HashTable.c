 ...
 * RA: 156331
 * Laboratório 10 - Hash - MC202 - Adaptado */
...
void free_HashT(HashT *t){
	...
	/* Percorre a tabela liberando memória quando necessário */
	for(i = 0; i < TAM_TABELA; i++){
		p = (*t)->l[i];
		while(p){
			temp = p;
...
/* Retorna se a chave está na hash-table */
bool is_in_HashT(HashT t, String key, char * c){
	return get_HashT(t, key, NULL, c);
}
...
/* Só retorna o tamnho da hash-table */
